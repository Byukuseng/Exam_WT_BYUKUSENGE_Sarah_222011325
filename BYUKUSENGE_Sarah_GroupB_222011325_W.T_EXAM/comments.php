...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO comments(comment_id, campaign_id, user_id, comment, timestamp) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("iiiss", $comment_id, $campaign_id, $user_id, $comment, $timestamp);
    ...
    <table border="3">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $comment_id = $row['comment_id']; // Fetch the comment_id
                echo "<tr>
