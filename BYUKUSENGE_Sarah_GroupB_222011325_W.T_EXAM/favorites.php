...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO favorites(favorite_id, campaign_id, user_id, timestamp) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("iiis", $favorite_id, $campaign_id, $user_id, $timestamp);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information of Favorites</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Favorites</h2></center>
    <table border="3">
        <tr>
            <th>Favorite ID</th>
            <th>Campaign ID</th>
        ...
        // Prepare SQL query to retrieve all favorites
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $favorite_id = $row['favorite_id']; // Fetch the favorite_id
                echo "<tr>
