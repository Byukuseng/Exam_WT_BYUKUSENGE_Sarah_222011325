...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <input type="number" id="like_id" name="like_id"><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO likes(like_id, campaign_id, user_id, timestamp) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("iiis", $like_id, $campaign_id, $user_id, $timestamp);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of Likes</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Likes</h2></center>
    <table border="3">
        <tr>
            <th>Like ID</th>
            <th>Campaign ID</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $like_id = $row['like_id']; // Fetch the like_id
                echo "<tr>
                    <td>" . $row['like_id'] . "</td>
                    <td>" . $row['campaign_id'] . "</td>
                    <td>" . $row['user_id'] . "</td>
                    <td>" . $row['timestamp'] . "</td>
                    <td><a style='padding:4px' href='delete_likes.php?like_id=$like_id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_likes.php?like_id=$like_id'>Update</a></td> 
                </tr>";
            }
        } else {
