...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO reports(report_id, campaign_id, user_id, reason) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("iiis", $report_id, $campaign_id, $user_id, $reason);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information of Reports</title>
    <style>
        table {
            width: 100%;
    ...
    <table border="3">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $report_id = $row['report_id']; // Fetch the report_id
                echo "<tr>
                    <td>" . $row['report_id'] . "</td>
