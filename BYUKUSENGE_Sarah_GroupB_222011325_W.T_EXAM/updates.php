...
<h1><u> Updates Form </u></h1>
<form method="post" onsubmit="return confirmInsert();">

    <label for="update_id">Update ID:</label>
    <input type="number" id="update_id" name="update_id"><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO updates(update_id, campaign_id, title, description, timestamp) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("iisss", $update_id, $campaign_id, $title, $description, $timestamp);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information of Updates</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Updates</h2></center>
    <table border="3">
        <tr>
            <th>Update ID</th>
            <th>Campaign ID</th>
        ...
        // Prepare SQL query to retrieve all updates
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $update_id = $row['update_id']; // Fetch the update_id
                echo "<tr>
                    <td>" . $row['update_id'] . "</td>
                    ...
                    <td>" . $row['timestamp'] . "</td>
                    <td><a style='padding:4px' href='delete_updates.php?update_id=$update_id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_updates.php?update_id=$update_id'>Update</a></td> 
                </tr>";
            }
